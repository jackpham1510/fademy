...
export const ApiEndpoint = {
  ...
  course: {
    ...
    increaseTotalView: (id: number) =>
      `/api/v1/course/increase-total-view/${id}`,
  },
  content: {
  ...
  studyProcess: {
    ...
    findByCourseId: (courseId: number) =>
      `/api/v1/study-process/find-by-course-id/${courseId}`,
    update: (courseId: number) => `/api/v1/study-process/${courseId}`,
  },
  watchList: {
    updateStatus: (courseId: number) =>
      `/api/v1/watch-list/update-status/${courseId}`,
  },
  mailSender: {
